...
3. **In the Python code**:
   - Use `await stimma.call_tool()` for each view with the same seed and character description
...
results = await asyncio.gather(*[stimma.call_tool(
...
- **Seed must be identical for every image** — pass the exact same constant to every `call_tool`. The prompt variation handles pose diversity; the fixed seed keeps the character's identity locked.
